...
This command will be run in each environment created.""",
    ...
    parser.add_option(
        ...
        dest="sn_dir",
        help=(
            "Create a snake nest (a dir containing symbolic links to each version-specific Python executable). "
            "The user should input the path to the desired dir and the program will create that directory if it "
            "does not exist. A warning is thrown if there are existing files in the given dir, and no snake nest "
            "will be generated."
        ),
    )
    parser.add_option(
...
def create_snake_nest(opts, pargs):
    ...
    for i, env_name in enumerate(env_names):
        ...
        if vers_spec_req is not None:
            subprocess.run(
                f"{env_manager} create -n {env_name} python={versions[i]} --file={vers_spec_req} --yes",
                shell=True,
            )
        else:
            subprocess.run(
                f"{env_manager} create -n {env_name} python={versions[i]} --yes",
                shell=True,
            )
        ...
        for i, env_name in enumerate(env_names):
            ...
            if opts.pip_reqs is not None or opts.dev_dir is not None:
                ...
                if vers_spec_preq is not None:
                    subprocess.run(
                        f"{env_manager} run -n {env_names[i]} pip install -r {vers_spec_preq}",
                        shell=True,
                    )
                if opts.dev_dir is not None:
                    subprocess.run(
                        f"{env_manager} run -n {env_names[i]} pip install -e {opts.dev_dir}",
                        shell=True,
                    )
            ...
            if sn_dir is not None:
                subprocess.run(
                    f"{env_manager} run -n {env_names[i]} which python >> {sn_file}",
                    shell=True,
                )
    ...
    for i, env_name in enumerate(env_names):
        with open(opts.script, "r") as rf:
            for command in rf:
                ...
                subprocess.run(
                    f"{env_manager} run -n {env_names[i]} {command}", shell=True
                )
