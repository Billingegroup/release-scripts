...
def create_option_parser():
    ...
    vsn_group.add_option("--version-bump", metavar="VBREGEX", dest="vb_regex")
    ...
    vsn_group.add_option(
        ...
        metavar="CATLIST",
        help=(
            "List of categories to include in the changelog. "
    ...
    vsn_group.add_option(
        ...
        metavar="APSTRING",
        help=(
            "All changes will be put in the change log after the access point. "
            "Default '.. current developments'.",
        ),
    )

    vsn_group.add_option(
        "--tag", action="store_true", help="Create and push a version tag to GitHub."
    )

    vsn_group.add_option(
        "--upstream", action="store_true", help="Push to upstream rather than origin."
    )
    ...
    rel_group.add_option(
        "--github", action="store_true", help="Initiate a release on GitHub."
    )

    rel_group.add_option("--gh-title", metavar="TITLE", help="Title of GitHub release.")
    ...
    rel_group.add_option(
        ...
        action="store_true",
        help=(
            "Initiate a release on PyPi. Default is to upload the source distribution. "
            "Use the --wheel option to also build a wheel using python-build.",
        ),
    )
...
def update_changelog(opts, pargs):
    ...
    if opts.upstream:
        remote = "upstream"
    remote_listing = call(
        f"git remote show {remote}", release_dir, capture_output=True
    ).stdout
...
def github_release(opts, pargs):
    ...
    # Release through gh
    call(
        f'gh release create "{version}" "./{tmp_dir}/{tgz_name}" "{gh_title}" "{gh_notes}"',
        release_dir,
    )
...
def pypi_release(opts, pargs):
    ...
    db_warning = (
        "Warning: No new distribution build. This occurs when there are no new changes "
        "to the source code since the previous release. Please check for any untracked "
        "changes and update your package changelog/release-history to reflect the "
        "newest version."
    )
    ...
    if build_wheel:
        ...
        if no_tar:
            ...
            call('echo "Warning: No wheel found."', release_dir)
        else:
            call(
                f"twine upload dist/*{version}*.tar.gz dist/*{version}*.whl",
                release_dir,
            )
...
def cf_hash(opts, pargs):
    ...
    dist_source = ""
    while dist_source.lower() not in sources and dist_source not in [
        str(i) for i in range(1, len(sources) + 1)
    ]:
    ...
    if dist_source == "2" or dist_source == "github":
        github_org = input(
            f"What organization/user's version of {module_name} are you looking for: "
        )
        source_url = (
            f"https://github.com/{github_org}/{module_name}/archive/{version}.tar.gz"
        )
        dist_source = "GitHub"
    ...
    if (
        ...
        warnings.warn(
            "SHA256 Hash Returned Emtpy File Hash. "
    ...
    with open(opts.forge, "w") as hash_dump:
        hash_dump.write(
            f"Distribution downloaded from: {source_url}\nSHA256: {sha256_hash}\n"
        )
...
if __name__ == "__main__":
    ...
    if opts.release and opts.pre_release:
        parser.error(
            "Both release and pre-release specified. "
            "Please re-run the command specifying either release or pre_release."
        )
